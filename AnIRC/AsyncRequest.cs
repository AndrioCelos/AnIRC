...
public abstract class AsyncRequest {
	...
	public class VoidAsyncRequest : AsyncRequest {
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			if (final) {
				if (!char.IsDigit(line.Message[0]) && this.nickname != null && line.Source != null &&
					!this.client.CaseMappingComparer.Equals(this.nickname, Hostmask.GetNickname(line.Source))) {
					// Wrong user.
					final = false;
					return false;
				}
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			switch (line.Message) {
				case RPL_WHOREPLY:
					...
					bool away = false, oper = false; IrcChannel? channel; ChannelStatus? channelStatus;

					if (line.Parameters[1] == "*") {
						channel = null;
						channelStatus = null;
					} else {
						channel = this.client.Channels.Get(line.Parameters[1]);
	...
	public class WhoxAsyncRequest : AsyncRequest {
		...
		private readonly List<WhoxResponse> responses = new();
		public ReadOnlyCollection<WhoxResponse> Responses { get; }

		private TaskCompletionSource<ReadOnlyCollection<WhoxResponse>> TaskSource { get; } = new TaskCompletionSource<ReadOnlyCollection<WhoxResponse>>();
		public string Target { get; }
		public string QueryType { get; }
		/// <summary>Returns a <see cref="Task{TResult}"/> of <see cref="ReadOnlyCollection{T}"/> of <see cref="WhoxResponse"/> representing the status of the request.</summary>
		public override Task Task => this.TaskSource.Task;
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			switch (line.Message) {
				case RPL_WHOSPCRPL:
					if (line.Parameters.Count != this.fields.Length + 1) return false;

					IrcChannel? channel = null; string? ident = null, host = null, ipAddress = null, server = null, nickname = null, account = null, fullName = null;
					bool away = false; bool oper = false; ChannelStatus? channelStatus = null; int hopCount = -1; int idleTime = 0;

					for (int i = line.Parameters.Count - 1; i >= 1; --i) {
						switch (this.fields[i - 1]) {
							case WhoxField.QueryType:
								if (line.Parameters[i] != this.QueryType) return false;
								break;
							case WhoxField.Channel:
								channel = line.Parameters[i] == "*" ? null : this.client.Channels.Get(line.Parameters[i]);
								break;
					...
					if (channel is not null) channelStatus ??= ChannelStatus.Empty;
					this.responses.Add(new(channel, ident, host, ipAddress, server, nickname, away, oper, channelStatus, hopCount, idleTime, account, fullName));
					break;
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			...
			switch (line.Message) {
				...
				case RPL_WHOISUSER:
					...
					this.providingServerName = line.Source;
					break;
	...
	public class CtcpAsyncRequest : AsyncRequest {
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			if (this.client.CaseMappingComparer.Equals(line.Message, "NOTICE")) {
				if (line.Parameters[1].Length >= 2 && line.Parameters[1].StartsWith("\u0001") && line.Parameters[1].EndsWith("\u0001") && line.Source is not null &&
					this.client.CaseMappingComparer.Equals(Hostmask.GetNickname(line.Source), this.target) &&
	...
	public class MessageAsyncRequest : AsyncRequest {
		...
		protected internal override bool OnReply(IrcLine line, ref bool final) {
			if (this.user.Client.CaseMappingComparer.Equals(Hostmask.GetNickname(line.Source ?? this.user.Client.ServerName!), this.user.Nickname) &&
