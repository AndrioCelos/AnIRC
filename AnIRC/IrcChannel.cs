...
namespace AnIRC {
	...
	public class IrcChannel : IrcMessageTarget, INotifyPropertyChanged {
		...
		public DateTime TopicStamp {
			get => this.topicStamp;
			set { this.topicStamp = value; this.OnPropertyChanged(new PropertyChangedEventArgs(nameof(this.TopicStamp))); }
		}
		...
		/// <param name="members">The users to affect.</param>
		public void DeHalfVoice(params string[] members) => this.SetMode('-', 'V', members);
		...
		/// <param name="members">The users to affect.</param>
		public void HalfVoice(params string[] members) => this.SetMode('+', 'V', members);
		...
		public void Join() => this.Client.Send("JOIN {0}", this.Name);
		/// <summary>Joins this channel using the specified key.</summary>
		/// <param name="key">The channel key.</param>
		public void Join(string key) => this.Client.Send("JOIN {0} {1}", this.Name, key);
		/// <summary>Kicks a user out of this channel.</summary>
		/// <param name="target">The user to kick.</param>
		public void Kick(string target) => this.Client.Send("KICK {0} {1}", this.Name, target);
		/// <summary>Kicks a user out of this channel with the specified message.</summary>
		/// <param name="target">The user to kick.</param>
		/// <param name="message">The kick message.</param>
		public void Kick(string target, string message) => this.Client.Send("KICK {0} {1} :{2}", this.Name, target, message);
		/// <summary>Kicks one or more users out of this channel.</summary>
		/// <param name="targets">The users to kick.</param>
		public void Kick(string[] targets) => this.Client.Send("KICK {0} {1}", this.Name, string.Join(",", targets));
		/// <summary>Kicks one or more users out of this channel with the specified message.</summary>
		/// <param name="targets">The users to kick.</param>
		/// <param name="message">The kick message.</param>
		public void Kick(string[] targets, string message) => this.Client.Send("KICK {0} {1} :{2}", this.Name, string.Join(",", targets), message);
		/// <summary>Leaves this channel.</summary>
		public void Part() => this.Client.Send("PART {0}", this.Name);
		/// <summary>Leaves this channel with the specified message.</summary>
		/// <param name="message">The part message.</param>
		public void Part(string message) => this.Client.Send("PART {0} :{1}", this.Name, message);
		/// <summary>Sends a message to this channel.</summary>
		/// <param name="message">What to say.</param>
		public override void Say(string message) => this.Client.Send("PRIVMSG {0} :{1}", this.Name, message);
		/// <summary>Sends a notice to this channel.</summary>
		/// <param name="message">What to say.</param>
		public override void Notice(string message) => this.Client.Send("NOTICE {0} :{1}", this.Name, message);
